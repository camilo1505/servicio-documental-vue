      ...
      <v-card>
        ...
        <v-card-text>
          <div class="embed-container" v-if="frame">
            <iframe width="1200" height="1000" :src="archivo" frameborder="0" allowfullscreen></iframe>
          </div>
          <form>
            <v-btn
            dark
        ...
        window.location.href=this.archivo.value;
          ...
          var src = Axios.get(this.url)
               .then(function (response) {
                     return response.request.responseURL; // the response.data is string of src
               })
